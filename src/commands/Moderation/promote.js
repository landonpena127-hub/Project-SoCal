...
export default {
  data: new SlashCommandBuilder()
    .setName('promote')
    .setDescription('Promote a BPT member to a new BPT rank')
    .addUserOption((option) =>
      option
        .setName('target')
        .setDescription('The BPT member to promote')
        .setRequired(true),
    )
    .addStringOption((option) =>
      option
        .setName('rank')
        .setDescription('The new BPT rank to give')
        .setRequired(true)
        .addChoices(
          ...BPT_RANKS.map((rank) => ({
            name: rank,
  ...
  async execute(interaction, config, client) {
    try {
      ...
      if (currentBPTRole && currentBPTRole.id === newRankRole.id) {
      ...
      if (currentBPTRole) {
        await member.roles.remove(currentBPTRole, `BPT Promotion | ${reason}`);
      }

      await member.roles.add(newRankRole, `BPT Promotion | ${reason}`);
      ...
      await InteractionHelper.universalReply(interaction, {
        embeds: [
          successEmbed(
            ...
            [
              `**Member:** ${user.tag}`,
              `**Previous Rank:** ${currentBPTRole ? currentBPTRole.name : 'None'}`,
